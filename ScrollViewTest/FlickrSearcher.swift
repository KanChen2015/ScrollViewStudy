...
let apiKey = "afa54de3b0c2c4b4f01670658c0d2a32"
...
class Flickr {
  ...
  func searchFlickrForTerm(searchTerm: String, completion : (results: FlickrSearchResults?, error : NSError?) -> Void){
    ...
    NSURLConnection.sendAsynchronousRequest(searchRequest, queue: processingQueue) {response, data, r in
        if r != nil {
            completion(results: nil,error: r)
            return
        }
